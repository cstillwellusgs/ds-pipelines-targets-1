# Read in data
data_for_plot <- read.csv("2_process/out/data_for_plot.csv")
...
for (mod in c('pb','dl','pgdl')){
  mod_data <- filter(data_for_plot, model_type == mod)
...
write_csv(data_for_plot, file = file.path("3_visualize/out", 'model_summary_results.csv'))


# Save the model diagnostics
render_data <- list(pgdl_980mean = filter(data_for_plot, model_type == 'pgdl', exper_id == "similar_980") %>% pull(rmse) %>% mean %>% round(2),
                    dl_980mean = filter(data_for_plot, model_type == 'dl', exper_id == "similar_980") %>% pull(rmse) %>% mean %>% round(2),
                    pb_980mean = filter(data_for_plot, model_type == 'pb', exper_id == "similar_980") %>% pull(rmse) %>% mean %>% round(2),
                    dl_500mean = filter(data_for_plot, model_type == 'dl', exper_id == "similar_500") %>% pull(rmse) %>% mean %>% round(2),
                    pb_500mean = filter(data_for_plot, model_type == 'pb', exper_id == "similar_500") %>% pull(rmse) %>% mean %>% round(2),
                    dl_100mean = filter(data_for_plot, model_type == 'dl', exper_id == "similar_100") %>% pull(rmse) %>% mean %>% round(2),
                    pb_100mean = filter(data_for_plot, model_type == 'pb', exper_id == "similar_100") %>% pull(rmse) %>% mean %>% round(2),
                    pgdl_2mean = filter(data_for_plot, model_type == 'pgdl', exper_id == "similar_2") %>% pull(rmse) %>% mean %>% round(2),
                    pb_2mean = filter(data_for_plot, model_type == 'pb', exper_id == "similar_2") %>% pull(rmse) %>% mean %>% round(2))

figure_1_caption <- 'resulted in mean RMSEs (means calculated as average of RMSEs from the five dataset iterations) of {{pgdl_980mean}}, {{dl_980mean}}, and {{pb_980mean}}°C for the PGDL, DL, and PB models, respectively.
